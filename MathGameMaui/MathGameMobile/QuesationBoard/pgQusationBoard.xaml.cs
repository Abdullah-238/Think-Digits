...
public partial class pgQusationBoard : ContentPage
{
    ...
    bool isMix = false;
    public pgQusationBoard(short numberOfQuestion, short level, short operation,bool IsMix)
    ...
    async void  CheckResult()
    {
        if (GameInfo.Result == GameInfo.UserInput)
        {
            ...
            await DisplayAlert("Great", "Your answer is correct", "Ok");
        }
        else
        {
            GameInfo.WrongAnswer++;
            //this.BackgroundColor = Colors.Red;
            await DisplayAlert("Wrong", "The correct answer is : " + GameInfo.Result, "Ok");
    ...
    private async void Button_Clicked(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(enInput.Text))
            Input = enInput.Text.Trim();
        else
        {
        ...
        {
            await Navigation.PushAsync(new pgGameInfo(GameInfo));
        }
        else
        {
